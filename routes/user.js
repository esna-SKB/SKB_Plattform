...
router.route('/')
	//get all users
	.get((req, res, next) => {
		User.find({},{_id:0, firstname:1, lastname:2, email:3, isTeacher:4, isAdmin:5, isValide:6,description:7, iCan:8, iLearn:9,iTeach:10,website:10}, function(err, users){
			if (err) {
...
router.route('/:email')
	...
	.get((req, res, next) => {
		...
		User.findOne({email: email},{_id:0, firstname:1, lastname:2, email:3, isTeacher:4, isAdmin:5, isValide:6,description:7, iCan:8, iLearn:9,iTeach:10,website:10}, function(err, user){
			if (err){
	           console.log('error occured in the database');
	           return res.status(500).send('error occured in the database');
			} else {
